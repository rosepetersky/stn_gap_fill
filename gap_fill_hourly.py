...
#gap_fill is used to fill hourly gap fill data 
...
def gap_fill_hourly(df,df_flags,df_inst):
    ...
    for stn in stnlist_uniques:
        ...
        if stn=="no stn":
            ...
            for i in range(len(stnlist_sub)):
                #Loop through the dataframe over a 12 hour period (gaps between 1-12 hours can be gap filled with linear interpolation)
                for j in range(0,len(df.iloc[:,1]),12):
                    ...
                    if np.isnan(df[stnlist_sub.iloc[i,0]][j]) and ~np.isnan(df[stnlist_sub.iloc[i,1]][j]):
                        #If j is greater than or equal to 720 hours before the end of the dataframe
                        if j>=(len(df)-720):
                        ...
                        else: #If j is not greater than or equal to 720 hours before the end of the dataframe 
                            #Get the index value of j
                            beg=df.index[j]
                            #Get the index value of j plus 720 hours 
                            end=df.index[j+720]
